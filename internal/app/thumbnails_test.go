...
func TestThumbnailsShowOnlyNearBottomWhileCursorIsActive(t *testing.T) {
	...
	if !v.shouldShowThumbnails(now, 1000-thumbnailRightExclusion-1, 650) {
		t.Fatal("pixel immediately left of right exclusion strip did not reveal thumbnails")
	}
	if v.shouldShowThumbnails(now, 1000-thumbnailRightExclusion, 650) {
		t.Fatal("right exclusion strip revealed thumbnails")
	}
	if v.shouldShowThumbnails(now, thumbnailLeftExclusion, 699) {
		t.Fatal("bottom-left horizontal mirror corner revealed thumbnails")
	}
	if v.shouldShowThumbnails(now, 999, 699) {
		t.Fatal("bottom-right gamma corner revealed thumbnails")
	}
